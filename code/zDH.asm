...
.DATA
    ...
    ; Decimal to Hexadecimal explanation messages
    var_DtoH_step1        DB "-> step 1: Take your number", 13, 10, "$"
    var_DtoH_step2        DB "-> step 2: Divide your number by 16, take the quotient, and divide it again", 13, 10, "$"
    var_DtoH_step3        DB "-> BE CAREFUL: Repeat until the final quotient is 0 (not float)", 13, 10, "$"
    var_DtoH_step4        DB "-> Number left of % its the quotient for each case", 13, 10, "$"
    var_DtoH_step5        DB "-> step 3: Put your residues together (from bottom to top)", 13, 10, "$"
    var_DtoH_step6        DB "-> BE CAREFUL: you don't need to include the quotient", 13, 10, "$"
    var_DtoH_step7        DB "-> step 4: The result of the plus it's your number in hexadecimal format!", 13, 10, "$"
    var_DtoH_arrow        DB "-> $"
    var_DtoH_space        DB " $"
    var_DtoH_final        DB "-> Your final number: $"
    var_DtoH_div          DB " / 16 = $"
    var_DtoH_mod          DB " % $"
    var_DtoH_openBracket  DB "[$"
    var_DtoH_closeBracket DB "]$"

    ; Buffer to store division remainders (only remainders needed for hex)
    DIVISION_REMAINDERS   DB 20 DUP(?)                                                                                     ; Space for up to 20 hex digits
...
SUBMAIN_DH PROC NEAR PUBLIC
    ...
    ; Step 1: Show the input number
                               LEA  DX, var_DtoH_step1
                               CALL PrintString
                               LEA  DX, var_DtoH_arrow
                               CALL PrintString
                               CALL PrintInputNumber
                               CALL PrintNewLine
                       
    ; Step 2: Show division explanation
                               LEA  DX, var_DtoH_step2
                               CALL PrintString
                               LEA  DX, var_DtoH_step3
                               CALL PrintString
                               LEA  DX, var_DtoH_step4
                               CALL PrintString
                               CALL PerformDivisions
                       
    ; Step 3: Show how to read the result
                               LEA  DX, var_DtoH_step5
                               CALL PrintString
                               LEA  DX, var_DtoH_step6
                               CALL PrintString
                       
    ; Step 4: Show final result
                               LEA  DX, var_DtoH_step7
                               CALL PrintString
                               LEA  DX, var_DtoH_final
                               CALL PrintString
...
SUBMAIN_DH ENDP
    ...
    ; ======= DECIMAL TO HEXADECIMAL EXPLANATION PROCEDURES =======
...
PerformDivisions PROC
    ...
    ; Print this division step
                               LEA  DX, var_DtoH_arrow
                               CALL PrintString
                               LEA  DX, var_DtoH_openBracket
                               CALL PrintString
                               
    ; Print: [number / 16 = quotient % remainder]
                               CALL Print32BitNumber              ; Print current number
                               LEA  DX, var_DtoH_div
                               CALL PrintString
                           
    ; Perform 32-bit division by 16
                               CALL Divide32BitBy16               ; Result: quotient in NUMBER_HIGH:NUMBER_LOW, remainder in DL
                           
    ; Store remainder for later hex construction
                               MOV  [DI], DL                      ; Store remainder (0-15)
    ...
    ; Print remainder as hex digit
                               MOV  AL, [DI-1]                    ; Get the remainder we just stored
                               CALL PrintHexDigit                 ; Print remainder as hex (0-F)
                               LEA  DX, var_DtoH_closeBracket
                               CALL PrintString
                               CALL PrintNewLine
...
Multiply32BitBy10 ENDP
    ...
    ; Divide 32-bit number by 16, return remainder in DL
Divide32BitBy16 PROC
                               PUSH AX
                               PUSH BX
    ...
    ; Get the remainder (bottom 4 bits of low word) BEFORE division
                               MOV  DL, BL
                               AND  DL, 15                        ; DL = remainder (0-15)
    ...
    ; Divide by 16 (shift right by 4)
                               ...
                               SHR  AX, 1                         ; High word / 16
                               RCR  BX, 1                         ; Low word / 16 with carry from high
...
Divide32BitBy10 ENDP
    ...
    ; Print the final hexadecimal result by reading remainders from last to first
PrintFinalHexResult PROC
    ...
    ; Print remainders from last to first to form hex number
    _PrintRemainderLoop:       
                               CMP  CL, 0
                               JE   _EndPrintHex
                               
                               MOV  AL, [SI]                      ; Get remainder (0-15)
                               CALL PrintHexDigit                 ; Convert and print as hex digit
...
PrintFinalHexResult ENDP
    ...
    ; Print a hex digit (0-9, A-F) given value in AL (0-15)
PrintHexDigit PROC
                               PUSH DX
                               CMP  AL, 9
                               JLE  _PrintDigit
