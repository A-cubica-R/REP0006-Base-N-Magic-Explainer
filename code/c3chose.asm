...
ApplyNumber PROC
                             ...
                             XOR  AX, AX                                   ; AX será nuestro acumulador parcial
                             XOR  DX, DX                                   ; DX:AX se usará si necesitamos extensión para valores grandes
                             XOR  BX, BX                                   ; BX será usado para valor temporal del dígito

                             MOV  CL, [BUFFER_IntputStr + 1]               ; Longitud del número
                             LEA  SI, BUFFER_IntputStr + 2                 ; Primer carácter

    _ConvLoop:               
                             CMP  CL, 0
                             JE   _Fin

                             MOV  AL, [SI]                                 ; AL = carácter ASCII actual
                             CALL ConvertCharToVal                         ; Retorna valor numérico en AL (usa base BL)
                             MOV  BL, AL                                   ; Guardar valor del dígito

    ; Multiplicar DX:AX por base (BL)
                             MOV  AX, WORD PTR BUFFER_IntputNum
                             XOR  DX, DX
                             MOV  CL, BL                                   ; base en CL
                             MOV  CH, 0
                             MOV  BX, CX                                   ; BX = base

    ; Multiplicación por base
                             MUL  BX                                       ; AX * BX → DX:AX

    ; Sumar el valor del dígito
                             ADD  AX, BX                                   ; BX = valor del dígito
    ; Guardar el nuevo valor en BUFFER_IntputNum
                             MOV  WORD PTR BUFFER_IntputNum, AX
