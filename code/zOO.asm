...
.DATA
    ...
    ; Octal to Octal explanation messages
    var_OtoO_step1 DB "-> Step 1: Your number is already in octal format!", 13, 10, "$"
    var_OtoO_step2 DB "-> Step 2: You don't need to make more steps", 13, 10, "$"
    var_OtoO_arrow DB "-> $"
    var_OtoO_space DB " $"
...
SUBMAIN_OO PROC NEAR PUBLIC
    ...
    ; Step 1: Show that number is already in octal format
                               LEA  DX, var_OtoO_step1
                               CALL PrintString
                               LEA  DX, var_OtoO_arrow
                               CALL PrintString
                               CALL PrintOctalDigitsSeparated
                               CALL PrintNewLine
                       
    ; Step 2: Show that no conversion is needed
                               LEA  DX, var_OtoO_step2
                               CALL PrintString
                               LEA  DX, var_OtoO_arrow
                               CALL PrintString
                               CALL PrintOctalAsIs
...
SUBMAIN_OO ENDP
    ...
    ; ======= OCTAL TO OCTAL EXPLANATION PROCEDURES =======
    
    ; Print octal digits separated by spaces
PrintOctalDigitsSeparated PROC
                               PUSH AX
                               PUSH BX
                               PUSH CX
                               PUSH DX
    ...
    _EndPrintDigits:           
                               ...
                               POP  AX
                               RET
PrintOctalDigitsSeparated ENDP

    ; Print octal number as is (without spaces)
PrintOctalAsIs PROC
                               PUSH AX
                               PUSH BX
                               PUSH CX
